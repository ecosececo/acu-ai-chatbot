...
- **Gemma 4 E4B** (via TGI) as the default local LLM (Ollama optional)
...
4. Retrieved context + question are sent to TGI (Gemma 4 E4B)
...
| LLM | Gemma 4 E4B (via TGI; Ollama optional) | Answer generation |
...
- Start Redis cache
- Start TGI and download the Gemma 4 E4B model (first time only)
...
- ✅ Local LLM (Gemma 4 E4B via TGI; Ollama optional)
...
│   │   │   ├── llm_service.py  # LLM provider (TGI/Ollama) + prompt engineering
...
### Model: Gemma 4 E4B (Instruction)
- **Parameters**: Effective 4B (8B total)
- **Serving**: TGI (HTTP API)
- **Why**: Strong Turkish output and long-context support with local inference
...
|----------|---------|-------------|
| `LLM_PROVIDER` | `tgi` | LLM provider: `tgi` or `ollama` |
| `TGI_BASE_URL` | `http://tgi-gemma4:80` | TGI endpoint |
| `HF_TOKEN` | (empty) | Optional Hugging Face token for gated models |
| `OLLAMA_BASE_URL` | `http://ollama:11434` | Ollama endpoint (embeddings) |
| `LLM_MODEL` | `google/gemma-4-E4B-it` | Model name (provider-specific) |
...
For TGI (Gemma family), update `.env`:
```bash
LLM_PROVIDER=tgi
...
For Ollama models, switch provider:
```bash
LLM_PROVIDER=ollama
...
3. Configure persistent volumes for PostgreSQL, Ollama, and TGI models
